"""
Helper functions to assist with determining the set of files to be used
for model training and model assessment. 
"""
import os
import glob
import subprocess
...
# same function is in the coquiTTS repo, but this version lets one 
# specify where WadaSNR is located
def compute_file_snr(file_path, wada_path='./'):
    ...
    os.system(command)
    command = [f'"{wada_path}/WadaSNR/Exe/WADASNR"', f'-i "{new_file}"', f'-t "{wada_path}/WadaSNR/Exe/Alpha0.400000.txt"', '-ifmt mswav']
...
# this function can take the place of the above, since the above need to run
# the binaries in /path/to/WadaSNR/Exe/WADASNR, which may not be allowed in 
# your docker env
def wada_snr(wav_path):
    ...
    wav = np.array(wav, dtype=object)[0]
    m = abs(wav).max()
    #return np.absolute(wav).all()
    wav = wav / m
    abs_wav = abs(wav)
    ...
    if type(snr) == np.array:
        return snr[0], wav_path
    else:
        return snr, wav_path
    #return type(snr)
